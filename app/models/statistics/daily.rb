module Statistics
  class Daily < BaseStats
    ...
    def get_stats table_name
      ...
      RankBase.connection.select_all("SELECT * FROM (SELECT Date(added_at) AS dd, rank FROM `#{table_name}` WHERE (ios_app_id = #{self.ios_app.id} AND added_at >= '#{self.begin_at.to_formatted_s(:db)}' AND added_at < '#{self.end_at.to_formatted_s(:db)}') ORDER BY rank ASC) AS tmp_table GROUP BY dd").each do |rb|
